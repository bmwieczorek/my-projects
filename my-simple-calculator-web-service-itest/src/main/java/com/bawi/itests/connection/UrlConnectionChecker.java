...
public class UrlConnectionChecker {
	...
	UrlConnectionChecker(int warmUpSeconds, int timeoutSeconds,
	...
	public UrlConnectionChecker() {
		this(DEFAULT_WARMUP_SECONDS, DEFAULT_TIMEOUT_SECONDS,
				DEFAULT_RETRY_INTERVAL_SECONDS);
	}

	public static UrlConnectionChecker create() {
		return new UrlConnectionChecker(DEFAULT_WARMUP_SECONDS,
				DEFAULT_TIMEOUT_SECONDS, DEFAULT_RETRY_INTERVAL_SECONDS);
	}
	...
	public static UrlConnectionChecker createWithTimeOutAndRertyInterval(
			int timeoutSeconds, int retryInverval) {
		return new UrlConnectionChecker(DEFAULT_WARMUP_SECONDS, timeoutSeconds,
				retryInverval);
	}

	public static UrlConnectionChecker createWithWarmUpAndTimeOutAndRertyInterval(
			int warmUpSeconds, int timeoutSeconds, int retryInverval) {
		return new UrlConnectionChecker(warmUpSeconds, timeoutSeconds,
				retryInverval);
	}
	...
	public boolean isUp(String url) throws Exception {
		return isUp(url, new ServiceValidityEvaluator() {
			...
			public boolean verifyExpession(HttpMethodBase httpMethod)
					throws Exception {
				return HttpStatus.SC_OK == httpMethod.getStatusCode()
						&& StringUtils.isNotEmpty(httpMethod
								.getResponseBodyAsString());
			}
		});
	}

	public boolean isUp(String url, ServiceValidityEvaluator evaluator)
			throws Exception {
		...
		for (int i = 0; i < timeoutSeconds; i = i + retryIntervalSeconds) {
			try {
				...
				logger.debug("Could not connect to " + url + " ("
	...
	private static void validateNotNegative(int warmUpSeconds) {
		if (warmUpSeconds < 0) {
			throw new IllegalArgumentException(
					"Warm up time cannot be negative: " + warmUpSeconds);
		}
	}
